...
const pages = ['O nas', 'Menu', 'Zamów Online', 'Galeria', 'Kontakt'];
...
function NavbarTop(props) {
  ...
  const routeChange=(e)=>{
    let path = e.target.innerText.toLowerCase().replace("ó", "o").replace(" ", "_");    
    navigate(path);
  }
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
            ...
            >
              ...
              {pages.map((page) => (
                ...
                underline="none"
                to={`${page.toLowerCase().replace("ó", "o").replace(" ", "-")}`}
                key={page.toLowerCase().replace("ó", "o").replace(" ", "-")}
                onClick={routeChange}
               >
              <MenuItem
               className={classes.MenuItems} key={page} onClick={handleCloseNavMenu}>
              {page}
              </MenuItem>
              </Link>
              ))}
